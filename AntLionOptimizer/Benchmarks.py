...
def f12(x):
    ...
    o = (math.pi / dim) * (
            ...
            + numpy.sum((((x[: dim - 1] + 1) / 4) ** 2) * (1 + 10 * (numpy.sin(math.pi * (1 + (x[1:] + 1) / 4))) ** 2))
            + ((x[dim - 1] + 1) / 4) ** 2
    ) + numpy.sum(u_fun(x, 10, 100, 4))
    return o
...
def f13(x):
    ...
    o = 0.1 * (
            ...
            + numpy.sum((x[:, :-1] - 1) ** 2 * (1 + (numpy.sin(3 * numpy.pi * x[:, 1:])) ** 2), axis=1)
            + ((x[:, -1] - 1) ** 2) *
            (1 + (numpy.sin(2 * numpy.pi * x[:, -1])) ** 2)
    ) + numpy.sum(u_fun(x, 5, 100, 4))
    return o
...
def f14(x):
    ...
    for i in range(0, 25):
        h = v - a_s[:, i]
        b_s[i] = numpy.sum((numpy.power(h, 6)))
    w = [i for i in range(25)]
    for i in range(0, 24):
        w[i] = i + 1
    o = ((1.0 / 500) + numpy.sum(1.0 / (w + b_s))) ** (-1)
    return o
...
def f15(i):
    ...
    a_k = numpy.asarray(a_k)
    b_k = numpy.asarray(b_k)
    b_k = 1 / b_k
    fit = numpy.sum(
...
def f17(i):
    o = (
            (i[1] - (i[0] ** 2) * 5.1 / (4 * (numpy.pi ** 2)) + 5 / numpy.pi * i[0] - 6)
            ** 2
            + 10 * (1 - 1 / (8 * numpy.pi)) * numpy.cos(i[0])
            + 10
    )
    return o
...
def f19(j):
    ...
    a_h = numpy.asarray(a_h)
    c_h = [1, 1.2, 3, 3.2]
    c_h = numpy.asarray(c_h)
    p_h = [
        [0.3689, 0.117, 0.2673],
        [0.4699, 0.4387, 0.747],
    ...
    p_h = numpy.asarray(p_h)
    o = 0
    for i in range(0, 4):
        o = o - c_h[i] * \
            numpy.exp(-(numpy.sum(a_h[i, :] * ((j - p_h[i, :]) ** 2))))
    return o
...
def f21(j):
    ...
    a_sh = numpy.asarray(a_sh)
    c_sh = numpy.asarray(c_sh)
    fit = 0
    for i in range(5):
        v = numpy.matrix(j - a_sh[i, :])
...
def ea_som(variables_values):
    ...
    func_value = -numpy.cos(x1) * numpy.cos(x2) * numpy.exp(-(x1 - numpy.pi) ** 2 - (x2 - numpy.pi) ** 2)
    return func_value
